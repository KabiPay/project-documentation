...
- Last updated: 2026-04-29 (**Session:** **M45.1 Payslip logo in PDF + compliance read for employees** — **`kabipay-common`**: HMAC file download token + public employee URL (**`file_download_token`**); **`kabipay-payroll`**: **`payslipLogoSignedReadUrl`** (only when **`fileStorageId`** matches **`payroll_compliance_setting.payslip_logo_file_storage_id`**); **`payrollComplianceSetting`** query readable by any authenticated client ( **`upsertPayrollComplianceSetting`** still statutory/HR-gated). **`kabipay-ui`**: signed URL fetch, header **`<img>`**, jsPDF **`addImage`** (PNG/JPEG); inline GraphQL doc **`documents.ts`** until gateway codegen. **`cargo check`** payroll + employee ✅ **`npm run build`** ✅. Prior **M45**: tax catalogue + **`0038`**/**`0039`**, etc.)
